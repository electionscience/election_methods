...
r = session.get('https://www.sos.wa.gov/elections/research/election-results-and-voters-pamphlets.aspx')
# print(r.html.links)
pattern= re.compile(r'(?!\/results\/)(\d{8})')
dates = {pattern.search(link)[0] for link in r.html.links if pattern.search(link)}

for date in dates:
    file_url = (f'https://results.vote.wa.gov/results/{date}/export/{date}_AllCounties.csv')
